...
const DevicePermissions: React.FC = () => {
    ...
    useEffect(() => {
        ...
        } else if (domainNameValue && domainNameValue !== '无') {
    ...
    const handleEdit = async (permissionId: string) => {
        try {
            ...
            if (response.code === 200 && response.data) {
                ...
                setValue('domainName', permission.domainStatusName ? permission.domainStatusName : '无'); // 使用权限数据中的域名
                setValue('domainGroup', permission.domainGroup);
                setValue('noDomainReason', permission.noDomainReason);
                // 转换 smartitStatus
                setValue('smartitStatus', permission.smartitStatusName ? permission.smartitStatusName : '未安装');
                setValue('noSmartitReason', permission.noSmartitReason);
                // 转换 usbStatus
                setValue('usbStatus', permission.usbStatusName ? permission.usbStatusName : '关闭');
                setValue('usbReason', permission.usbReason || '');
                setValue('useEndDate', permission.usbExpireDate ? dayjs(permission.usbExpireDate) : null);
                // 转换 antivirusStatus
                setValue('connectionStatus', permission.antivirusStatusName ? permission.antivirusStatusName : '无连接');
    ...
    const onSubmitForm = async (data: PermissionFormData) => {
        try {
            ...
            const permissionData: DevicePermissionInsert = {
                ...
                domainStatusId: data.domainName ?
                (data.domainName === '无' ? 50 :
                    data.domainName === 'D1' ? 51 :
                        data.domainName === 'D2' ? 52 :
                            data.domainName === 'D3' ? 53 :
                                data.domainName === 'D4' ? 54 :
                                    data.domainName === 'D5' ? 55 :
                                        data.domainName === 'D6' ? 56 :
                                            data.domainName === 'D7' ? 57 :
                                                data.domainName === 'EU' ? 58 :
                                                    data.domainName === 'MG' ? 59 :
                                                        data.domainName === 'EQU' ? 60 :
                                                            data.domainName === 'NRI-01' ? 61 :
                                                                data.domainName === 'MS' ? 62 : 1) : 10, // 下拉框选择的domainName映射到传给后端的domainStatusId
                domainStatusName: data.domainName,
                domainGroup: data.domainGroup,
                noDomainReason: data.domainName === '无' ? data.noDomainReason : '',
                // 转换 smartitStatus: '本地'->63, '远程'->64, '未安装'->65，其他->null
                smartitStatusId: data.smartitStatus === '本地' ? 63 :
                    data.smartitStatus === '远程' ? 64 :
                        data.smartitStatus === '未安装' ? 65 : null,
                smartitStatusName: data.smartitStatus, // 保存文本值
                noSmartitReason: data.noSmartitReason,
                // 转换 usbStatus: '数据'->67, '3G网卡'->68, '关闭'->66，其他->null
                usbStatusId: data.usbStatus === '数据' ? 67 :
                    data.usbStatus === '3G网卡' ? 68 :
                        data.usbStatus === '关闭' ? 66 : null,
                usbStatusName: data.usbStatus, // 保存文本值
                ...
                antivirusStatusId: data.connectionStatus === '自动' ? 70 :
                    data.connectionStatus === '手动' ? 71 :
                        data.connectionStatus === '无连接' ? 69 : null,
                antivirusStatusName: data.connectionStatus, // 保存文本值
    ...
    const columns: ColumnsType<DevicePermissionList> = [
        ...
        },
        {
            ...
            key: 'domainStatus',
            width: 120,
            render: (domainStatus: number) => {
                if (domainStatus === 38) {
                    return '未参加';
                } else if (domainStatus === 39) {
                    return '参加済み';
                } else {
                    return domainStatus || '-';  // 其他值显示原始值或'-'
                }
            },
        },
        {
        ...
        },
        {
            ...
            key: 'smartitStatus',
            width: 120,
            render: (smartitStatus: number) => {
                // 优先使用存储的文本值
                const status = smartitStatus;

                if (status) {
                    // 根据文本值设置颜色
                    if (status === 41) {
                        return <Tag color="green">インストール済み</Tag>
                    } else if (status === 40) {
                        return <Tag color="red">未インストール</Tag>;
                    }else {
                        return <Tag>-</Tag>;
                    }
                }
        ...
        },
        {
            ...
            key: 'usbStatus',
            width: 100,
            render: (usbStatus: number) => {
                const status = usbStatus;
                if (status) {
                    // 根据文本值设置颜色
                    if (status === 43) {
                        return <Tag color="orange">一時許可</Tag>
                    }
                    if (status === 42) {
                        return <Tag color="red">禁止</Tag>;
                    }
                    if (status === 44){
                        return <Tag color="green">許可</Tag>;
                    }
                    else {
                        return <Tag>-</Tag>;
                    }
        ...
        },
        {
            ...
            key: 'antivirusStatus',
            width: 120,
            render: (antivirusStatus: number) => {
                const status = antivirusStatus;
                if (status) {
                    // 根据文本值设置颜色
                    if (status === 45) {
                        return <Tag color="orange">有効期限切れ</Tag>
                    }
                    if (status === 46) {
                        return <Tag color="red">未インストール</Tag>;
                    }
