...
import type { DeviceFullDTO, DeviceTableData, DeviceQueryParams, PageResponse, DictItem, UserInfo } from '../types/devices';
  ...
  async getDevices(params: DeviceQueryParams): Promise<PageResponse<DeviceTableData>> {
    ...
    if (params.deviceId) {
      filteredData = filteredData.filter(item => 
        item.computerName.includes(params.deviceId!) ||
        item.deviceId.includes(params.deviceId!)
      );
    }
    if (params.userId) {
