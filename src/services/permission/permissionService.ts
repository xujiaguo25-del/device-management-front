    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            const excelData = mockPermissions.map(p => [
                ...
                p.deptId || '',
                p.loginUsername,
                p.domainStatusName || '', // Domain状态显示domainName
                p.domainGroup || '',
                p.smartitStatusName || '', // SmartIT状态使用文本值
                p.usbStatusName || '', // USB状态使用文本值
                p.usbExpireDate || '', // USB过期日期
                p.antivirusStatusName || '', // 防病毒状态使用文本值
                p.remark || '', // 备注
            ]);
